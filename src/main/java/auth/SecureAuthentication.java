    ...
    public AuthenticationStatus validateRequest(HttpServletRequest request, HttpServletResponse response, HttpMessageContext ctx) throws AuthenticationException {
        ...
        try {
            if (request.getRequestURI().contains("Logout")) {
                ...
                request.logout();
                KeepRecord.reset();
                response.sendRedirect("/event_management/Login.jsf");
...
//                
                ...
                if (result.getStatus() == Status.VALID) {
                    ...
                    RegisterMaster user = sl.getUserByUserName(username);
                    ...
                    if (result.getCallerGroups().contains("admin")) {
                        ...
                        response.sendRedirect("Admin/dashbord.jsf");
//                        request.getRequestDispatcher("Admin/EventDisplay.jsf").forward(request, response);
                    }
                    if (result.getCallerGroups().contains("student")) {
//                        request.getRequestDispatcher("Student/home.jsf").forward(request, response);
                          response.sendRedirect("Student/home.jsf");
                    }
